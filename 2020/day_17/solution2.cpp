...
public:
  ...
  Coordinate(int i1, int i2, int i3, int i4) {
    x = i1;
    y = i2;
    z = i3;
    w = i4;
  ...
  if (this->x == otherPoint.x && this->y == otherPoint.y && this->z == otherPoint.z && this->w == otherPoint.w) return true;
  else return false;
  }
};
...
namespace std {
    template <> struct hash<Coordinate> {
        size_t operator()(const Coordinate & x) const {
            ...
            std::string s = "";
            s = to_string(x.x)  + "-" + to_string(x.y) + "-" + to_string(x.z) + "-" + to_string(x.w);
            return h(s);
...
public:
  ...
  std::pair<int64_t, int64_t> bounds_w;
  int generation;

  Cube(std::unordered_set<Coordinate> active, int x1, int x2, int y1, int y2, int z1, int z2, int w1, int w2) {
    active_squares = active;
    bounds_x = std::make_pair(x1, x2);
    bounds_y = std::make_pair(y1, y2);
    bounds_z = std::make_pair(z1, z2);
    bounds_w = std::make_pair(w1, w2);
    generation = 0;
  }
  ...
  void generate_next_step() {
    ...
    bounds_w = std::make_pair(bounds_w.first - 1, bounds_w.second+1);
  ...
  void update_bounds() {
    ...
    int min_x{0}, min_y{0}, min_z{0}, min_w{0};
    int first_loop = 1;
    for(const Coordinate c : active_squares) {
      ...
      max_w = (c.w > max_w) ? c.w : max_w; min_w = (c.w < min_w) ? c.w : min_w;
    }
    ...
    bounds_w.first = min_w; bounds_w.second = max_w;
  }
    ...
    for(const Coordinate &c : active_squares) {
      for(int i=-1;i<=1;i++) {
        for(int j=-1;j<=1;j++) {
          for(int k=-1;k<=1;k++) {
            for(int l=-1;l<=1;l++) {
              Coordinate new_coord = Coordinate(c.x+i, c.y+j, c.z+k, c.w+l);;
      ...
      for(int j=-1;j<=1;j++) {
        for(int k=-1;k<=1;k++) {
          for(int l=-1;l<=1;l++) {
            ...
            Coordinate nbr = Coordinate(c.x+i, c.y+j, c.z+k, c.w+l);
    ...
    for(int i=bounds_z.first; i<=bounds_z.second;i++) {
      ...
      for(int j=bounds_x.first; j<=bounds_x.second;j++) {
        for(int k=bounds_y.first; k<=bounds_y.second;k++) {
          Coordinate coord = Coordinate(j,k,i,l);
...
int main(void) {
  ...
  while(getline(input, line)) {
    ...
    for(char &c : line) {
      if(c == '#') {
        Coordinate coord = Coordinate(line_num, char_num, 0,0);
  ...
  // Part 2
  Cube cube = Cube(initial_active, 0, max_x-1, 0, line_num-1, 0,0,0,0);
