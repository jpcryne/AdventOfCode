...
int main(void) {
  std::vector<int> cups(1000000);
  std::vector<int> cups_init = {1,2,3,4,8,7,5,9,6};

  for(int i=0;i<cups_init.size();i++) {
    cups.at(i) = cups_init.at(i);
  }
  for(int i=cups_init.size();i<cups.size();i++) {
    cups.at(i) = i;
  }
  std::cout << "Cups Initialised\n";
  //std::vector<int> cups_init = {3, 8,  9,  1,  2,  5,  4,  6,  7 };
