...
Rule parse_rule(std::string s) {
  ...
  auto tmp_index = s.find('|');
  if(tmp_index != std::string::npos) {
    std::stringstream ss1(s.substr(0, tmp_index));
    int i;
    ...
    std::stringstream ss2(s.substr(tmp_index+1, s.size()));
    while (ss2 >> i) {
      o2.push_back(i);
    }
...
std::pair<bool, int> follows_rule(std::string s, int rule, std::unordered_map<int, Rule> &m) {
  ...
  //std::cout << s << ": " << make_string(r) << std::endl;
  if(s.size() == 0) {
    sum = 0;
  } else if(r.rule_type == RuleType::letter_rule) {
    b = (s.at(0) == r.letter);
    sum = 1;
...
bool check_message(std::string s, std::unordered_map<int, Rule> &m) {
  ...
  std::cout << std::endl;
  std::cout << ((pr.first) ? "TRUE, " : "FALSE, ") << "String size: " << s.size() << " = sum_size: " << pr.second  << std::endl;
  return pr.first;
}
...
int main(void) {
  ...
  for(auto elem : messages) {
    //std::cout << elem << ", ";
    bool tmp = check_message(elem, rule_map);
    if(tmp) std::cout << elem << std::endl;
    sum+=tmp;
  }
